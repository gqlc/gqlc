...
func verifyPositions(t *testing.T, fset *DocSet, f *Doc, lines []int) {
...
func TestPositions(t *testing.T) {
	...
	for _, test := range tests {
		...
		f := fset.AddDoc(test.filename, fset.Base()+delta, test.size)
...
func TestLineInfo(t *testing.T) {
	fset := NewDocSet()
	f := fset.AddDoc("foo", fset.Base(), 500)
...
func TestFiles(t *testing.T) {
	...
	for i, test := range tests {
		...
		fset.AddDoc(test.filename, base, test.size)
		j := 0
		fset.Iterate(func(f *Doc) bool {
...
func TestFileSetPastEnd(t *testing.T) {
	fset := NewDocSet()
	for _, test := range tests {
		fset.AddDoc(test.filename, fset.Base(), test.size)
	}
	if f := fset.Doc(Pos(fset.Base())); f != nil {
...
func TestFileSetRace(t *testing.T) {
	fset := NewDocSet()
	for i := 0; i < 100; i++ {
		fset.AddDoc(fmt.Sprintf("file-%d", i), fset.Base(), 1031)
	}
	max := int32(fset.Base())
...
`)
	...
	const filename = "foo"
	fset := NewDocSet()
	f := fset.AddDoc(filename, fset.Base(), len(src))
	f.SetLinesForContent(src)
